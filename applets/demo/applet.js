/* =================================================================
   ...
   independent Matter.js rigid body floating in zero gravity. Moving
   the cursor repels the letters; invisible static perimeter walls
   keep every letter on-screen.
...
const META = { id: 'zero-gravity-readme', title: 'Zero-Gravity README' };
...
function build() {
  ...
  root.innerHTML = `
    <div class="zg__topbar">
      ...
      <span class="zg__name">Zero-Gravity README</span>
  ...
  const activate = async () => {
    ...
    enterImmersive(); // honoured because we're inside a click handler
    const Matter = await loadMatter();
    // Let layout settle after the fullscreen transition before measuring.
...
function startSimulation({ Matter, stage, readmeEl, canvas, fpsMeter, bodyMeter }) {
  ...
  const T = 80; // thick so fast bodies can't tunnel through
  ...
  runtime.stopLoop = ctx.loop.start((dt) => {
    ...
    draw(g2d, W, H, letters, mouse, REPEL_RADIUS);
...
function draw(g2d, W, H, letters, mouse, repelRadius) {
